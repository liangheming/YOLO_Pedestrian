...
class GaussTaylorKeyPointDecoder(BasicKeyPointDecoder):
    ...
    @torch.no_grad()
    def __call__(self, heat_map: torch.Tensor, trans_inv: torch.Tensor, return_heat_map=False):
        ...
        return (trans_output, max_val) if not return_heat_map else (trans_output, max_val, ret_heat_map)
