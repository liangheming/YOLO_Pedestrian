...
class BoxInfo(object):
    ...
    def __init__(self, img_path, shape=None, boxes=None, labels=None, weights=None):
        ...
        self.PADDING_VAL = 114

    def set_padding_val(self, val):
        self.PADDING_VAL = val
    ...
    def load_img(self):
        if self.img is None:
            self.img = cv.imread(self.img_path).astype(np.float64)
        return self
    ...
    def draw_box(self, colors, names):
        ...
        # ret_img[ret_img[..., 0] == self.PADDING_VAL, :] = 125
...
class MixUp(BasicTransform):
    ...
    def aug(self, box_info: BoxInfo) -> BoxInfo:
        ...
        temp_img = np.ones(shape=(max(h1, h2), max(w1, w2), 3), dtype=np.float64) * box_info.PADDING_VAL
